...
class Dejavu:
    ...
    def fingerprint_file(self, file_path: str, song_name: str = None) -> None:
        """
        Fingerprints a single file without relying on multiprocessing or _fingerprint_worker.

        :param file_path: Path to the file.
        :param song_name: Optional name to associate with the song.
        """
        ...
        # Directly fingerprint the file
        fingerprints, file_hash = self.get_file_fingerprints(file_path, self.limit, print_output=True)

        # Store results in the database
        sid = self.db.insert_song(song_name, file_hash, len(fingerprints))
        if not sid:
            raise ValueError(f"❌ insert_song() failed for '{song_name}' — check for DB issues.")
        else:
            print(f"✅ Inserted song '{song_name}' with ID: {sid}")
        self.db.insert_hashes(sid, fingerprints)
        self.db.set_song_fingerprinted(sid)
        self.__load_fingerprinted_audio_hashes()
        print(f"🧠 Fingerprint count for {song_name}: {len(fingerprints)}")
