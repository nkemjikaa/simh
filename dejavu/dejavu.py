    ...
    if args.fingerprint:
        ...
        if len(args.fingerprint) == 2:
            ...
            if isdir(filepath):
                ...
                sys.exit(1)
            print(f"Fingerprinting file: {filepath}")
            try:
                djv.fingerprint_file(filepath)
                print("✅ Fingerprinting complete")
            except TypeError as e:
                print(f"❌ TypeError: {e}. Trying fallback method without song_name.")
                try:
                    from dejavu.djv_modules.fingerprint import fingerprint_file
                    fingerprint_file(filepath, config_file)
                    print("✅ Fallback fingerprinting complete")
