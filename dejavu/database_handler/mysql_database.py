...
class MySQLDatabase(CommonDatabase):
    ...
    def insert_hashes(self, song_id, hashes):
        """
        Inserts the fingerprint hashes for a given song into the database.

        :param song_id: ID of the song.
        :param hashes: List of (hash, offset) tuples.
        """
        values = [(song_id, h[0], h[1]) for h in hashes]
        insert_query = """
...
class Cursor(object):
    ...
    def __enter__(self):
        ...
        return self.cursor  # ✅ return the actual DB cursor
